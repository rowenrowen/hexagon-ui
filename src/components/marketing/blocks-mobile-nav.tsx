...
/**
 * Mobile `/blocks/*`: section tabs + customize preview — aligned to `site-grid`
 * gutters (same track as header and block frames).
 */
  ...
  return (
    <div className="sticky top-14 z-20 border-b border-border/70 bg-background/95 backdrop-blur-lg lg:hidden">
      <div className="site-grid">
        <nav aria-label="Block categories" className="w-full pt-1">
            ...
            {HUB_BLOCK_SECTIONS.map((section) => {
              ...
              return (
                <Link
                  ...
                  className={
                    active
                      ? "-mb-px shrink-0 border-b-2 border-primary px-3.5 py-3.5 text-sm font-semibold text-foreground transition-colors"
                      : "-mb-px shrink-0 border-b-2 border-transparent px-3.5 py-3.5 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"
                  }
                >
                  {section.title}
                </Link>
        ...
        <details className="group border-t border-border/50">
          <summary className="flex h-14 min-h-14 cursor-pointer list-none items-center justify-between gap-2 text-sm font-medium text-muted-foreground marker:hidden [&::-webkit-details-marker]:hidden">
